...
export default function Home() {
  return (
    <div className="space-y-10">
      <section className="rounded-3xl bg-gradient-to-r from-indigo-700 via-purple-700 to-pink-600 p-8 text-white shadow-2xl">
        ...
        <Link href="/products" className="mt-6 inline-block rounded-full bg-white px-6 py-3 font-bold text-slate-900">Shop Now</Link>
      </section>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {categories.map((category) => (
            <Link key={category.name} href={`/products?category=${encodeURIComponent(category.name)}`} className="rounded-2xl bg-white p-5 shadow hover:shadow-lg">
              <p className="text-xs font-bold uppercase text-pink-600">{category.discount}</p>
              <p className="mt-2 text-xl font-black text-slate-900">{category.name}</p>
            </Link>
          ))}
        </div>
      </section>
